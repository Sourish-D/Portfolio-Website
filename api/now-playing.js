export default async function handler(req, res) {
  ...
  if (!apiKey || !username) {
    return res.status(500).json({ error: "Missing Last.fm environment variables" });
  }
  ...
  try {
    ...
    const [recentRes, infoRes, artistsRes, tracksRes, albumsRes] = await Promise.all([
      fetch(recentUrl),
      fetch(infoUrl),
      fetch(topArtistsUrl),
      fetch(topTracksUrl),
    ...
    const track = {
      ...
      albumImageUrl: latestTrack?.image?.[latestTrack.image.length - 1]?.['#text'] || null,
    ...
    const topAlbumImg = (albumImages && Array.isArray(albumImages) && albumImages.length > 0)
    ...
    return res.status(500).json({ error: "Failed to fetch music data from Last.fm" });
  }
}